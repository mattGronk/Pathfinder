...
export default function ProfileForm({ profile, enabled }: { profile: CareerProfile | null; enabled: boolean }) {
  ...
  const [already18,setAlready18] = useState(false);
  const needsGuardian = !!birthYear && (currentYear-Number(birthYear)<18 || (currentYear-Number(birthYear)===18&&!already18));
  ...
  return <>
    ...
    <form className="profile-form" action={enabled ? action : undefined} onSubmit={(event) => {
      ...
      <fieldset disabled={pending || erasing}><legend>1. Your starting point</legend>
        ...
        <label>Birth year<Input name="birthYear" type="number" required min={1900} max={currentYear} step={1} value={birthYear} onChange={e=>{setBirthYear(e.target.value);setAlready18(false);}} placeholder="For example: 2009" /></label>{currentYear-Number(birthYear)===18&&<label className="check-label"><input type="checkbox" name="already18" checked={already18} onChange={e=>setAlready18(e.target.checked)}/>I have already had my 18th birthday.</label>}
      ...
      <fieldset disabled={pending || erasing}><legend>3. What would make this useful?</legend><label>Your main career question<Textarea name="goal" required minLength={5} maxLength={500} defaultValue={profile?.goal} placeholder="For example: How can I combine my interest in business with technology?" /></label>
        {needsGuardian&&<div className="guardian-consent"><h3>Parent or guardian consent</h3><p>A parent or legal guardian must complete this section before the learner’s information is saved. We store the profile, answers and results to provide private career guidance. Reports are shared only when you choose. Consent can be withdrawn through the account contact listed in our privacy notice.</p><label>Parent or guardian’s full name<Input name="guardianName" required minLength={2} maxLength={100}/></label><label>Relationship<NativeSelect name="guardianRelationship" required defaultValue=""><NativeSelectOption value="" disabled>Select relationship</NativeSelectOption><NativeSelectOption value="parent">Parent</NativeSelectOption><NativeSelectOption value="guardian">Legal guardian</NativeSelectOption></NativeSelect></label><label className="check-label"><Checkbox name="guardianConsent" required/><span>I am the parent or legal guardian named above, legally competent to consent for this learner. I have read the privacy notice and consent to the described storage and use of their profile, assessment answers and results.</span></label></div>}
      ...
      <div aria-live="polite">{state.error && <p role="alert" className="form-error">{state.error}</p>}{state.message && <p className="form-success">{state.message} <Link href="/dashboard?view=assessments">Continue to my assessment centre →</Link></p>}{previewed && <p className="form-success">Preview complete. This is where your profile will be saved when you sign in. Nothing has been saved.</p>}</div>
